...
export default function IssueEditPage() {
    ...
    const fetch = async () => {
        ...
        (service.User.id === userProfile.userId && !issue)
        setService(service);
        setModos(modos)
    ...
    return (
        <form onSubmit={formik.handleSubmit} className=" Body gray gap-3 pb-2">
            ...
            <header className="px-4">
                <NavBarTop />
                <SubHeader type={"Conciliation"} place={` sur ${service?.type === ServiceType.GET ? "une demande" : "une offre"} de service  ${userProfile?.userId === service?.userId ? "que j'ai créé" : "à laquelle j'ai repondu"}`} closeBtn />
            </header>
            {loading ?
