...
import { Address, Profile, User } from '../../types/class';
...
export default function MyInfosPage() {
    ...
    }, [formik.values.Address, imgBlob, user])
    ...
    const fetch = async () => {
        ...
        formik.values.address = `${Profile.Address.address} ${Profile.Address.zipcode} ${Profile.Address.city}`
        formik.values.firstName = Profile.firstName
        formik.values.lastName = Profile.lastName
        formik.values.phone = Profile.phone
        formik.values.assistance = Profile.assistance.toString()
        formik.values.skills = Profile.skills.toString().split(',')
        formik.values.image = Profile.image
        setSkillList(formik.values.skills)
    ...
    async function addressIn() {
        ...
        addressFind.id !== newUser.addressId && (formik.values.addressId = addressFind.id);
                ...
                <main className='flex flew-1 pt-8'>
                    <Card className="w-respLarge h-full justify-between">
                        <CardHeader className="bg-transparent shadow-none flex pb-2 justify-center items-end " floated={true}>
                            <Avatar src={imgBlob ? imgBlob as string : user.image as string} alt="avatar" size="xl" className=" shadow-md w-24 h-24" />
                        ...
                        <CardBody className="flex flex-1 flex-col h-full gap-[4%] pb-4 pt-0 overflow-auto !max-h-[calc(100vh-18rem)]">
                            <Typography className='text-xs'>Information pour : {user.firstName} </Typography>
