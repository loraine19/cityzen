...
export default function FlagEditPage() {
    ...
    const [img] = useState<any>(image);
    const [imgBlob, setImgBlob] = useState<any>(img);
    const [open, setOpen] = useState(false);
    const index = data.issues.findIndex((element: Issue) => element.id === issue.id);
    function saveIssue() {
        data.issues[index] = (formik.values as Issue)
                        ...
                        <CardHeader className={"FixCardHeader NoImage"}
                            ...
                            <div className={` "absolute top-0 p-2" h-full   justify-between w-full flex `}>
                                ...
                                <Typography className=' px-2'>
                                    je créé ce litige sur {service.type === "get" ? "une demande" : "une offre"} de service  {user.id === service.user_id ? "que j'ai créé" : "à laquelle j'ai repondu"}</Typography>
